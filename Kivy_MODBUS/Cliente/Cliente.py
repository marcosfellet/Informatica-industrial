    ...
    def escreveDado(self, tipo, addr, valor):
        ...
        if tipo == 1:
            if (valor.is_integer()):
                ...
                lista_registrador = self.float(valor)
                print("\nbrengs:\n", lista_registrador[0], lista_registrador[1])
                return self._cliente.write_multiple_registers(addr,lista_registrador)


        if tipo == 2:
            return self._cliente.write_single_coil(addr,valor)
    
    def float(self, valor):
        # Caso para valores float
        bytes = struct.pack('>f', valor)
        #registradores de 16 bites cada
        reg_1, reg_2 = struct.unpack('>HH', bytes)
        return [reg_1, reg_2]

class Cliente_modbus_kivy(ClienteMODBUS):
    def atendimento(self, port, id):
        self._cliente.open()
        while True:
            self.__port = input('\nDigite o endereço da porta da conexão: ')
            self.__id = input('\nDigite o enderção de IP do servidor: ')
